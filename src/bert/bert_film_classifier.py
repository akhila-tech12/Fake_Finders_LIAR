...
if __name__ == "__main__":
    ...
    # -- Train ---------------------------------------------------------------
    clf = BERTFiLMClassifier(epochs=3, batch_size=16, lr=2e-5)
    clf.fit(TRAIN, VALID)
    ...
    # -- Comparison ---------------------------------------------------------------
    print(f"\n{'='*60}")
    print("  COMPARISON -- Does FiLM help more than concatenation?")
    print(f"{'='*60}")
    print(f"  BERT text-only             : F1 = 0.6724")
    print(f"  BERT + metadata (concat)   : F1 = 0.6743")
    print(f"  BERT + FiLM (this script)  : F1 = {results['f1']:.4f}")
    diff_vs_concat = results['f1'] - 0.6743
    sign1 = "+" if diff_vs_concat >= 0 else ""
    print(f"  FiLM vs concatenation      : {sign1}{diff_vs_concat:.4f}")
    print(f"  {'-'*56}")
    print(f"  MLP (TF-IDF + metadata)    : F1 = 0.7542  <- current best")
    diff_vs_mlp = results['f1'] - 0.7542
    sign2 = "+" if diff_vs_mlp >= 0 else ""
    print(f"  FiLM vs MLP                : {sign2}{diff_vs_mlp:.4f}")
    print(f"{'='*60}")
